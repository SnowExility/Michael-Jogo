...
const GP = {
  ...
  btnY: false, btnY_prev: false,    // Y (Xbox) / Triangle (PS) = inventory
...
function pollGamepad() {
  ...
  GP.btnY_prev = GP.btnY; GP.btnY = gp.buttons[3]?.pressed || false;
...
function applyGamepadInput() {
  ...
  const btnB_edge  = GP.btnB  && !GP.btnB_prev;
  const btnX_edge  = GP.btnX  && !GP.btnX_prev;
  const btnY_edge  = GP.btnY  && !GP.btnY_prev;
  const start_edge = GP.btnStart && !GP.btnStart_prev;
  const now = Date.now();
  const axisUp   = GP.leftY < -0.4 && GP.leftY_prev >= -0.4;
  const axisDown = GP.leftY >  0.4 && GP.leftY_prev <=  0.4;
  const navUp    = (GP.dUp   && !GP.dUp_prev)   || (axisUp   && now - GP.lastNavTime > 200);
  const navDown  = (GP.dDown && !GP.dDown_prev) || (axisDown && now - GP.lastNavTime > 200);
  const navLeft  = GP.dLeft  && !GP.dLeft_prev;
  const navRight = GP.dRight && !GP.dRight_prev;
  if (navUp || navDown) GP.lastNavTime = now;

  // ── CUTSCENE ──
  if (gState === 'CUTSCENE') {
    if (btnA_edge || navRight) cutsceneNext();
    if (navLeft)               cutscenePrev();
    return;
  }

  // ── INVENTORY ──
  if (gState === 'INVENTORY') {
    if (btnY_edge) toggleInventoryPanel(); // Y again = close
    if (btnA_edge) invPanelUse();
    if (btnB_edge) toggleInventoryPanel(); // B = close too
    return;
  }

  // ── OVERLAYS (shop / menu / confirm) ──
  ...
  // ── GAMEPLAY ──
  // Movement (axis + dpad)
  const moveLeft  = GP.leftX < -GP.DZ || GP.dLeft;
  const moveRight = GP.leftX >  GP.DZ || GP.dRight;
...
function updateGamepadIcons() {
  const el = document.getElementById('gamepad-hints');
  if (!el) return;
  if (!GP.connected || !GP.showIcons) { el.classList.add('hidden'); return; }
  el.classList.remove('hidden');

  const isPS = GP.type === 'ps';
  const title = document.getElementById('gph-title');
  if (title) title.textContent = isPS ? 'DUALSHOCK' : 'XBOX';

  // Jump button: A (xbox green) / × (ps)
  const btnJump = document.getElementById('gph-btn-jump');
  if (btnJump) {
    btnJump.textContent = isPS ? '×' : 'A';
    btnJump.className = 'gph-btn gph-a';
  }
  // Interact: X (xbox blue) / □ (ps pink)
  const btnInteract = document.getElementById('gph-btn-interact');
  if (btnInteract) {
    btnInteract.textContent = isPS ? '□' : 'X';
    btnInteract.className = isPS ? 'gph-btn gph-x-ps' : 'gph-btn gph-x-xbox';
  }
  // Bag/Inventory: Y (xbox) / △ (ps)
  const btnBag = document.getElementById('gph-btn-bag');
  if (btnBag) {
    btnBag.textContent = isPS ? '△' : 'Y';
    btnBag.className = isPS ? 'gph-btn gph-y-ps' : 'gph-btn gph-y-xbox';
  }
  // Menu: Start
  const btnMenu = document.getElementById('gph-btn-menu');
  if (btnMenu) { btnMenu.textContent = '≡'; btnMenu.className = 'gph-btn gph-start'; }

  // Update interact hint inline
  const hintEl = document.getElementById('interact-hint');
  if (hintEl && !hintEl.classList.contains('hidden')) {
    const hText = document.getElementById('interact-text');
    if (hText) hText.textContent = isPS ? '□  Interagir' : 'X  Interagir';
  }
  // Update cutscene hint
  const csHint = document.getElementById('cutscene-gp-hint');
  if (csHint) {
    csHint.textContent = isPS ? '× avançar  ← voltar' : 'A avançar  ← voltar';
...
// ═══ WIND WAVES (pixel art horizontal streaks) ═══
const windWaves = [];
let nextWindTime = 2000 + Math.random() * 4000;
  ...
  if (nextWindTime <= 0) {
    ...
    for (let i = 0; i < count; i++) {
      windWaves.push({
        ...
        h: Math.max(1, Math.round(1 + Math.random() * 2)),
        segments: Math.floor(2 + Math.random() * 3),
      });
    }
    nextWindTime = 3000 + Math.random() * 6000;
  }
  for (let i = windWaves.length - 1; i >= 0; i--) {
    windWaves[i].x += windWaves[i].speed;
    if (windWaves[i].x > MAP_W + 300) windWaves.splice(i, 1);
  }
}

function drawWindWaves(camX) {
  windWaves.forEach(w => {
    const sx = w.x - camX;
    if (sx > CW + 50 || sx + w.len < -50) return;
    ctx.save();
    ctx.globalAlpha = w.alpha;
    ctx.fillStyle = 'rgba(255,255,255,0.9)';
    // Draw as pixel art dashes
    const ps = Math.max(1, w.h);
    const segLen = Math.round(w.len / w.segments);
    const gap = Math.round(segLen * 0.4);
    for (let s = 0; s < w.segments; s++) {
      const dx = sx + s * (segLen + gap);
      ctx.fillRect(Math.round(dx), Math.round(w.y), segLen - gap, ps);
    }
    ctx.restore();
  });
}
const cloudImgs = [null, null, null]; // cloud1.png, cloud2.png, cloud3.png
function preloadCloudImages() {
  [1,2,3].forEach((n,i) => {
    const img = new Image();
    img.onload = () => { cloudImgs[i] = img; };
    img.src = `assets/images/cloud${n}.png`;
  });
}
...
function initClouds(){
  preloadCloudImages();
  clouds=[];
  [{spd:.04,y:35,n:6},{spd:.09,y:80,n:8},{spd:.16,y:128,n:7}].forEach(c=>{
    for(let i=0;i<c.n;i++) clouds.push({
      x:Math.random()*(MAP_W*1.4), y:c.y+(Math.random()-.5)*28,
      w:70+Math.random()*140, spd:c.spd,
      alpha:.55+Math.random()*.35,
      variant: Math.floor(Math.random()*3), // which PNG to use
    });
  });
}
...
function drawClouds(camX){
  ...
  clouds.forEach(cl=>{
    ...
    const cImg = cloudImgs[cl.variant];
    if(cImg){
      ctx.drawImage(cImg, sx, cl.y, cl.w, cl.w * (cImg.height/cImg.width));
    } else {
      drawCloudPixel(sx, cl.y, cl.w, cl.variant);
    }
    ctx.restore();
  });
}

// Pixel art cloud — 3 variants matching pixel art style of sky.png
function drawCloudPixel(x, y, w, variant){
  const ps = Math.max(2, Math.round(w/18)); // pixel size
  const rows = [
    // variant 0: wide fluffy
    [[3,2],[2,4],[4,3],[6,3],[8,4],[10,3],[12,4],[14,3],[16,2]],
    // variant 1: tall puffy
    [[4,2],[3,4],[5,4],[7,5],[9,5],[11,4],[13,4],[15,2]],
    // variant 2: long flat
    [[2,2],[4,3],[6,3],[8,3],[10,3],[12,3],[14,3],[16,2]],
  ];
  const shape = rows[variant % 3];
  const baseX = x + ps;
  const baseY = y;
  ctx.fillStyle = 'rgba(255,255,255,0.95)';
  shape.forEach(([col, height]) => {
    for(let row=0; row<height; row++){
      ctx.fillRect(baseX + col*ps, baseY + (4-height+row)*ps, ps, ps);
    }
  });
  // shadow row
  ctx.fillStyle = 'rgba(180,210,240,0.35)';
  shape.forEach(([col]) => ctx.fillRect(baseX + col*ps, baseY + 4*ps, ps, ps));
}

// ═══ BACKGROUND (mais árvores, troncos marrons) ═══
// ═══ BACKGROUND IMAGES (PNG se existir, senão procedural) ═══
const bgImgs = { sky: null, sun: null };
// Enemy sprite images: tomato(2), lettuce(2), carrot(2)
const enemyImgs = {
  tomato:  [null, null],       // tomato_walk0.png, tomato_walk1.png
  lettuce: [null, null],       // lettuce_fly0.png,  lettuce_fly1.png
  carrot:  [null, null],       // carrot_idle.png,   carrot_charge.png
};
function preloadBgImages() {
  const tryLoad = (key, src) => {
    const img = new Image();
    img.onload = () => { bgImgs[key] = img; };
    img.src = src;
  };
  tryLoad('sky', 'assets/images/sky.png');
  tryLoad('sun', 'assets/images/sun.png');
  // Enemy sprites
  [['tomato',['tomato_walk0','tomato_walk1']],['lettuce',['lettuce_fly0','lettuce_fly1']],['carrot',['carrot_idle','carrot_charge']]].forEach(([type,names])=>{
...
function drawBG(camX){
  ...
  // ── SKY ── (PNG se tiver, senão gradiente)
  if (bgImgs.sky) {
    ctx.drawImage(bgImgs.sky, 0, 0, CW, GROUND);
  } else {
  ...
  // ── SUN ── (PNG se tiver, senão procedural)
  const sunX=CW*.78, sunY=GROUND*.16;
  if (bgImgs.sun) {
    // Glow halo behind the PNG
    ctx.save();
    ctx.globalAlpha=.22+.08*Math.sin(t*.002);
    const sg=ctx.createRadialGradient(sunX,sunY,8,sunX,sunY,90);
    sg.addColorStop(0,'#FFFFA0');sg.addColorStop(1,'rgba(255,255,100,0)');
    ctx.fillStyle=sg;ctx.fillRect(sunX-90,sunY-90,180,180);
    ctx.restore();
    const size = 80;
    ctx.drawImage(bgImgs.sun, sunX - size/2, sunY - size/2, size, size);
  } else {
    ctx.save();ctx.globalAlpha=.2;
    const sg=ctx.createRadialGradient(sunX,sunY,8,sunX,sunY,90);
    sg.addColorStop(0,'#FFFFA0');sg.addColorStop(1,'rgba(255,255,100,0)');
    ctx.fillStyle=sg;ctx.fillRect(sunX-90,sunY-90,180,180);ctx.restore();
...
function drawBGTree(x, w, h, col) {
  const ps = Math.max(1, Math.round(w / 20)); // pixel size for art style
  const tw = Math.max(ps*2, Math.round(w * .14));
  const th = Math.round(h * .3);
  const cx = Math.round(x);
  const top = Math.round(GROUND - h);

  // ── TRUNK (orange-brown like in sky.png) ──
  const trunkColors = ['#C84A10', '#E05A18', '#A03808'];
  // Left/shadow
  ctx.fillStyle = trunkColors[2];
  ctx.fillRect(cx - tw/2, GROUND - th, Math.ceil(tw*0.35), th);
  // Main
  ctx.fillStyle = trunkColors[0];
  ctx.fillRect(cx - Math.ceil(tw*0.35) + cx - tw/2, GROUND - th, Math.ceil(tw*0.65), th);
  // Full trunk
  ctx.fillStyle = trunkColors[0];
  ctx.fillRect(Math.round(cx - tw/2), Math.round(GROUND - th), tw, th);
  // Highlight stripe
  ctx.fillStyle = trunkColors[1];
  ctx.fillRect(Math.round(cx - tw/2 + tw*0.3), Math.round(GROUND - th), Math.max(1, Math.round(tw*0.25)), th);
  // Dark left
  ctx.fillStyle = trunkColors[2];
  ctx.fillRect(Math.round(cx - tw/2), Math.round(GROUND - th), Math.max(1, Math.round(tw*0.2)), th);
  // Bark rings
  ctx.fillStyle = 'rgba(60,20,0,0.3)';
  for(let i=1; i<4; i++) {
    ctx.fillRect(Math.round(cx-tw/2), Math.round(GROUND-th*i/4), tw, Math.max(1,ps));
  }

  // ── CANOPY — pixel art triangular tiers like the image ──
  // Number of tiers scales with tree height
  const tiers = Math.max(3, Math.min(5, Math.round(h / 60)));
  const darkOutline = darken(col, 30);
  const midCol = col;
  const lightCol = lighten(col, 14);
  const shadowCol = darken(col, 15);

  for (let t = 0; t < tiers; t++) {
    const tFrac = t / tiers;
    // Each tier: progressively narrower at top, slight overlap
    const tierBot = GROUND - th - (h - th) * tFrac;
    const tierH   = (h - th) / tiers * 1.3; // slight overlap
    const tierTop = tierBot - tierH;
    const halfW   = w * 0.5 * (1 - tFrac * 0.55);

    // Pixel-snapped triangle with stepped edges (pixel art look)
    const steps = Math.max(4, Math.round(halfW / ps));
    for (let s = 0; s <= steps; s++) {
      const frac = s / steps;
      const rowY = Math.round(tierTop + (tierBot - tierTop) * frac);
      const rowW = Math.round(halfW * 2 * frac);
      const rowX = Math.round(cx - rowW / 2);
      const rowH = Math.max(ps, Math.round((tierBot - tierTop) / steps * 1.1));
      ...
      ctx.fillRect(rowX, rowY, Math.max(1, Math.round(rowW * 0.3)), rowH);

      // Shadow (right 25%)
      ctx.fillStyle = shadowCol;
      ctx.fillRect(rowX + Math.round(rowW * 0.75), rowY, Math.max(1, Math.round(rowW * 0.25)), rowH);
    }

    // Dark outline on the tier bottom edge
    const bW = Math.round(halfW * 2);
    ctx.fillStyle = darkOutline;
    ctx.fillRect(Math.round(cx - bW/2), Math.round(tierBot - ps), bW, Math.max(1, ps));
    // Bottom row extra dark pixels (step effect)
    ctx.fillStyle = darkOutline;
    const bStepW = Math.max(1, Math.round(bW * 0.08));
    ctx.fillRect(Math.round(cx - bW/2 - bStepW), Math.round(tierBot - ps*2), bStepW*2, ps*2);
    ctx.fillRect(Math.round(cx + bW/2 - bStepW), Math.round(tierBot - ps*2), bStepW*2, ps*2);
  }

  // Tip pixel
  ctx.fillStyle = lightCol;
  ctx.fillRect(cx - ps, top, ps*2, ps*2);
}
...
function spawnEnemies(){
  enemies=ENEMY_DEFS.filter(d=>d.x<SAFE.x1||d.x>SAFE.x2).map(d=>{
    ...
    else if(d.t==='carrot') Object.assign(e,{x:d.x,y:GROUND-72,w:52,h:72,charging:false,chargeVx:0,chargeLeft:0,dir:-1,originX:d.x,chargeSpeed:3.5});
    return e;
  });
}
...
function drawMapNuggets(camX) {
  ...
  mapNuggets.forEach(n => {
    ...
    const sy = n.y + bob;
    const glow = .25 + .18 * Math.abs(Math.sin(t * .004 + n.phase));
    ctx.save();
    // Glow halo
    ctx.globalAlpha = glow;
    const rg = ctx.createRadialGradient(sx, sy, 2, sx, sy, 20);
    rg.addColorStop(0, '#FF9900'); rg.addColorStop(1, 'rgba(255,153,0,0)');
    ctx.fillStyle = rg; ctx.fillRect(sx - 20, sy - 20, 40, 40);
    ctx.restore();
    // Emoji nugget 🍗
    ctx.save();
    ctx.globalAlpha = 1;
...
// ═══ INVENTORY PANEL ═══
function toggleBag() { toggleInventoryPanel(); }  // keep old alias
...
function toggleInventoryPanel() {
  ...
  if (panel.classList.contains('hidden')) {
    ...
    if (gState === 'INVENTORY') gState = 'PLAYING';
  }
}

function renderInventoryPanel() {
  // Slot 0
  const icon = document.getElementById('inv-ps-icon-0');
  const name = document.getElementById('inv-ps-name-0');
  const desc = document.getElementById('inv-ps-desc-0');
  const slot = document.getElementById('inv-ps-0');
  const useBtn = document.getElementById('inv-use-btn');

  if (P.item === 'potion') {
    icon.textContent = '⚡';
    name.textContent = 'POÇÃO';
    desc.textContent = '30s de invulnerabilidade';
    slot.classList.add('inv-selected');
    slot.classList.remove('inv-empty');
    if (useBtn) useBtn.disabled = false;
  } else {
    icon.textContent = '—';
    name.textContent = 'VAZIO';
    desc.textContent = '';
    slot.classList.remove('inv-selected');
    slot.classList.add('inv-empty');
    if (useBtn) useBtn.disabled = true;
  }

  // Close hint
  const closeHint = document.getElementById('inv-panel-close-hint');
  if (closeHint) {
    if (GP.connected) {
      closeHint.textContent = GP.type === 'ps' ? '△ fechar' : 'Y fechar';
    } else {
      closeHint.textContent = '[Q] fechar';
    }
  }

  // Panel hint
  const hint = document.getElementById('inv-panel-hint');
  if (hint) {
    if (GP.connected) {
      hint.textContent = GP.type === 'ps' ? '× usar · △ fechar' : 'A usar · Y fechar';
    } else {
      hint.textContent = 'Clique USAR ou pressione Q para fechar';
    }
  }
...
function invPanelUse() {
  if (P.item === 'potion') {
    P.potion = {timeLeft: P.POTION_DUR};
    P.item = null;
    updateInventoryUI();
    document.getElementById('invul-bar-wrap').classList.remove('hidden');
    sfxBuy();
...
function updateHUDLives(){
  [1,2,3].forEach(i=>{
    document.getElementById(`heart${i}`)?.classList.toggle('lost',i>P.lives);
    document.getElementById(`mob-heart${i}`)?.classList.toggle('lost',i>P.lives);
  });
}
function updateHUDNuggets(){
  const val = (readNuggets()+sessionNuggets).toLocaleString('pt-BR');
  document.getElementById('hud-nuggets').textContent = val;
  const mob = document.getElementById('mob-nugget-count');
...
function openIngameMenu(){
  ...
  igmTab('game'); igmFocusIdx=0; updateIGMFocus();
...
// ═══ CUTSCENE ═══
// Auto-detect available cutscene images (tries .png first, falls back to .svg)
// Only includes images that exist — game preloads and filters
const CUTSCENE_BASE = [
  'assets/images/cutscene/fase1/scene1',
  'assets/images/cutscene/fase1/scene2',
  'assets/images/cutscene/fase1/scene3',
  'assets/images/cutscene/fase1/scene4',
  'assets/images/cutscene/fase1/scene5',
];
let CUTSCENE_IMGS = [];
let cutsceneImgsLoaded = false;

function loadCutsceneImgs(cb) {
  if (cutsceneImgsLoaded) { cb(); return; }
  let done = 0;
  const results = [];
  CUTSCENE_BASE.forEach((base, i) => {
    // Try .png first
    const img = new Image();
    img.onload  = () => { results[i] = base + '.png'; check(); };
    img.onerror = () => {
      // Try .svg fallback
      const img2 = new Image();
      img2.onload  = () => { results[i] = base + '.svg'; check(); };
      img2.onerror = () => { results[i] = null; check(); };
      img2.src = base + '.svg';
    };
    img.src = base + '.png';
  });
  function check() {
    ...
    if (done === CUTSCENE_BASE.length) {
      CUTSCENE_IMGS = results.filter(Boolean);
      cutsceneImgsLoaded = true;
      cb();
    }
  }
}
...
function startCutscene(){
  ...
  // Load image list first, then show
  loadCutsceneImgs(() => {
    cutscenePage = 0;
    renderCutscene();
  });
...
function renderCutscene(){
  if (!CUTSCENE_IMGS.length) {
    // No images found — show text fallback and allow skipping
    document.getElementById('cutscene-img').style.display = 'none';
    const box = document.getElementById('cutscene-box');
    if (!document.getElementById('cutscene-fallback')) {
      const fb = document.createElement('div');
      fb.id = 'cutscene-fallback';
      fb.style.cssText = 'color:#FFD700;font-size:12px;text-align:center;padding:20px;line-height:2.5;font-family:"Press Start 2P",monospace;';
      fb.innerHTML = '🍞 PARABÉNS!<br><br>Você encontrou o Pão!<br><br>Coloque as imagens em:<br>assets/images/cutscene/fase1/<br><br>scene1.png ... scene5.png';
      box.insertBefore(fb, document.getElementById('cutscene-controls'));
    }
    document.getElementById('cutscene-next').textContent = 'FINALIZAR ✔';
    document.getElementById('cutscene-prev').disabled = true;
  ...
  img.src = CUTSCENE_IMGS[cutscenePage];
  ...
  if (GP.connected) updateGamepadIcons();
}
function cutsceneNext(){
  const total = CUTSCENE_IMGS.length || 1; // 1 so empty goes straight to finish
  if(cutscenePage < total-1){ cutscenePage++; renderCutscene(); } else finishPhase();
}
...
function loop(ts){
  ...
  if(gState==='PLAYING'||gState==='INVENTORY'){
    if(gState==='PLAYING'){
      updatePhysics(dt);
      updateEnemies(dt);
      checkInteractables();
...
function showPhaseIntro() {
  const el = document.getElementById('phase-intro');
  if (!el) return;
...
function init(){
  ...
  // Enter on code input
  document.getElementById('igm-code-input')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') igmRedeemCode();
  });

  // Q = toggle inventory
  document.addEventListener('keydown', e => {
    if ((e.key === 'q' || e.key === 'Q') && (gState === 'PLAYING' || gState === 'INVENTORY')) {
      toggleInventoryPanel();
    }
  });

  // Arrow keys for cutscene
  document.addEventListener('keydown', e => {
    if (gState !== 'CUTSCENE') return;
    if (e.key === 'ArrowRight' || e.key === ' ') cutsceneNext();
